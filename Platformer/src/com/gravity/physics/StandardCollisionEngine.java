...
public class StandardCollisionEngine implements CollisionEngine {
    ...
    private static final float ON_GROUND_OFFSET = 5f;
    private static final Transform ON_GROUND_TRANSFORM = Transform.createTranslateTransform(0, ON_GROUND_OFFSET);
    
    public StandardCollisionEngine(GameWorld map) {
    ...
    /**
     * @see com.gravity.physics.CollisionEngine#addEntity(com.gravity.entity.Entity)
     */
    @Override
    ...
    /**
     * @see com.gravity.physics.CollisionEngine#removeEntity(com.gravity.entity.Entity)
     */
    @Override
    ...
    public boolean isOnGround(Entity entity, float millis) {
        Shape hitbox = entity.getCurrentPhysicalState().snapshot(millis).getShape().transform(ON_GROUND_TRANSFORM);
        boolean onGround = false;
        for (Entity terrain : terrainCallColls) {
            if (hitbox.intersects(terrain.getShape(millis))) {
                // it's safe to pass null arguments because enemies shouldn't care what points were hit
                terrain.handleCollisions(millis, Lists.newArrayList(new Collision(entity, terrain, millis, null, null)));
                onGround = true;
            }
        }
        if (onGround) {
        ...
        for (Entity terrain : terrainNoCalls) {
            if (hitbox.intersects(terrain.getShape(millis))) {
                return true;
            }
        }
    ...
    /**
     * @see com.gravity.physics.CollisionEngine#update(int)
     */
    @Override
    public void update(float millis) {
        ...
        Preconditions.checkArgument(millis > 0, "Number of millis must be positive: millis=" + millis);
        
        float increment = millis / PARTS_PER_TICK;
        ...
        for (float time = increment; time <= millis; time += increment) {
            ...
            if (!collisions.isEmpty()) {
                for (Entry<Entity, List<Collision>> c : collisions.entrySet()) {
                    Entity e = c.getKey();
                    e.setPhysicalState(e.handleCollisions(increment, c.getValue()));
                }
                collisions = computeCollisions(increment);
                ...
                if (!collisions.isEmpty()) {
                    for (Entry<Entity, List<Collision>> c : collisions.entrySet()) {
                        Entity e = c.getKey();
                        e.setPhysicalState(e.rehandleCollisions(increment, c.getValue()));
                    }
                    collisions = computeCollisions(increment);
                }
                
                if (!collisions.isEmpty()) {
                    throw new RuntimeException("Could not properly rehandle collisions!");
                }
            }
            for (Entity e : entities) {
                e.setPhysicalState(e.getPhysicalState(millis));
                e.updated(millis);
            }
        }
    ...
    static boolean isIntersecting(float[] p1, float[] p2, float[] p3, float[] p4) {
        ...
        unknownA = (((p4[0] - p3[0]) * (double) (p1[1] - p3[1])) - ((p4[1] - p3[1]) * (p1[0] - p3[0]))) / (((p4[1] - p3[1]) * (p2[0] - p1[0])) - ((p4[0] - p3[0]) * (p2[1] - p1[1])));
        unknownB = (((p2[0] - p1[0]) * (double) (p1[1] - p3[1])) - ((p2[1] - p1[1]) * (p1[0] - p3[0]))) / (((p4[1] - p3[1]) * (p2[0] - p1[0])) - ((p4[0] - p3[0]) * (p2[1] - p1[1])));
    ...
    // Package private for testing
    static Collision collisionLines(Entity entityA, Entity entityB, float time) {
    ...
    private static void getShapeIntersections(Shape a, Shape b, Map<Integer, List<Integer>> aCollisions, Map<Integer, List<Integer>> bCollisions) {
        // @formatter:off
        /*
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            for (int j = 0; j < thatLength; j += 2) {
                ...
                unknownA = (((points[iNext] - points[i]) * (double) (thatPoints[j + 1] - points[i + 1])) - ((points[iNext + 1] - points[i + 1]) * (thatPoints[j] - points[i]))) / (((points[iNext + 1] - points[i + 1]) * (thatPoints[jNext] - thatPoints[j])) - ((points[iNext] - points[i]) * (thatPoints[jNext + 1] - thatPoints[j + 1])));
                unknownB = (((thatPoints[jNext] - thatPoints[j]) * (double) (thatPoints[j + 1] - points[i + 1])) - ((thatPoints[jNext + 1] - thatPoints[j + 1]) * (thatPoints[j] - points[i]))) / (((points[iNext + 1] - points[i + 1]) * (thatPoints[jNext] - thatPoints[j])) - ((points[iNext] - points[i]) * (thatPoints[jNext + 1] - thatPoints[j + 1])));
